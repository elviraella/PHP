...
$query = $db->query("SELECT * FROM news WHERE category='it_jobs'");
...
foreach($row as $r){
	...
	$_SESSION['valid_user'] = "user_id";
		...
		$news_id = $r['news_id'];
		$query2 = $db->query("SELECT * FROM comments WHERE jobs_id='$news_id'");
		...
		if(isset($_POST['commentAdd'])){
			...
			$sql ="INSERT INTO comments (comment_id, comment, user_id, news_id, jobs_id, devices_id) VALUES (null, '".$_POST['comment']."', '".$user_id."', 0, '".$news_id."',0)";
			#echo $sql;
			$db->exec($sql);	
