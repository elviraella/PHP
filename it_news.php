...
$query = $db->query("SELECT * FROM news WHERE category='it_news'");
$row = $query->fetchAll(PDO::FETCH_ASSOC);
	foreach($row as $r){
		...
		echo "<h3>Commnents</h3>";
		$news_id = $r['news_id'];
		$query2 = $db->query("SELECT *  FROM comments WHERE news_id='$news_id'");
		...
		?>
			<form action="" method="POST">
				...
				<textarea  type="text" name="comment" rows="4" cols="50" ></textarea><br>
	...
	if(isset($_POST['commentAdd'])){
    	...
    	$sql ="INSERT INTO comments (comment_id, comment, user_id, news_id, jobs_id, devices_id ) VALUES (null, '".$_POST['comment']."', '".$user_id."', '".$news_id."', 0, 0)";
    	#echo $sql;
		$db->exec($sql);
